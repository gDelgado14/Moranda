...
var ConsoleLogger = require('./console_logger')
...
function Botkit (configuration) {
  var botkit = {
    ...
    tasks: [],
    bots: {}, // this will hold all bot instances being managed accross all teams
    taskCount: 0,
    convoCount: 0
  }
  ...
  botkit.utterances = {
    ...
    no: new RegExp(/^(no|nah|nope|n|cancel)/i)
  }
  ...
  ********/
  ...
  /**
   * Make sure we don't connect to RTM twice for the same team
   * @param  {Object} bot  - The bot worker instance to be tracked
   * @return {Undefined}   - No explicit return
   */
  botkit.trackBot = function (bot) {
    botkit.bots[bot.config.token] = bot
  }
  ...
  // this method should not be accessible by a developer
  // this is a utility function for Slackbot_worker.startConversation
  ...
  botkit.defineBot = function (unit) {
    // unit is an instance of Slackbot_worker.js
  ...
  botkit.worker = function (botkit, config) {
    ...
    this.replyWithQuestion = function (message, question, cb) {
      ...
      // botkit.startConversation expects 'bot' as first argument!!
