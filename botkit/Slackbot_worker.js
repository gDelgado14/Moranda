...
var slackWebApi = require('./Slack_web_api.js')
var HttpsProxyAgent = require('https-proxy-agent')
var Back = require('back')
// var fs = require('fs') // for debugging purposes
...
module.exports = function (botkit, config) {
  ...
  bot.configureRTM = function (config) {
    console.log('Slackbot_worker.js - config')
    console.log(config)
  ...
  bot.startRTM = function (cb) {
    bot.api.rtm.start({
      ...
      /*****************************
      // DEBUGGING OUTPUT
      // save file to disk for inspecting purposes
      console.log('========== bot.startRTM - res ==========')
      fs.writeFile('res.json', JSON.stringify(res, null, 4), e => {
      ...
      ****************************/
      ...
      // https://github.com/websockets/ws/blob/master/doc/ws.md#class-wswebsocket
      // Ws is an event emitter
      ...
      bot.rtm.on('open', function () {
        ...
        bot.rtm.on('message', function (data, flags) {
          ...
          try {
            ...
            console.log('========= bot.rtm message event fired =========')
            console.log(data)
          } catch (err) {
  ...
  bot.send = function (message, cb) {
    ...
    if (message.attachments || message.icon_emoji ||
      ...
      if (!bot.config.token) {
        throw new Error('Missing token to connect to Web API.')
      }
      ...
      if (!bot.rtm)
        throw new Error('Bot missing RTM configuration. Spawn with bot token.')
  ...
  // send json response from Slash Command request
  // everyone will see this response
  // https://api.slack.com/slash-commands
  bot.replyPublic = function (src, resp, cb) {
    // when called via SlackBot.handleSlackEvents, bot has no token attribute.
    // this may be the reason why slash commands are responding with the `slash-command` identity
  ...
  // send json response from Slash Command request
  // only the user who submitted the Slash Command
  ...
  // reply in the same channel as src.channel
