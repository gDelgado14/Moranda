...
function Slackbot (configuration) {
  ...
  // customize the bot definition, which will be used when new connections spawn!
  slack_botkit.defineBot(require('./Slackbot_worker.js'))
   ...
   * @param  {[obj]}   slack_app_config  - configuration object containing `clientId`, `clientSecret`, `redirectUri` and `scopes`
   * @param  {Function} cb               [description]
   * @return {[obj]}   coreBot                 [description]
   */
  slack_botkit.configureSlackApp = function (slack_app_config, cb) {
    slack_botkit.log('** Configuring app as a Slack App!')

    // need to provide clientId, clientSecret, & authentication scopes
  ...
  function secureWebhookEndpoints () {
    var authenticationMiddleware = require('./middleware/slack_authentication.js')
  ...
  slack_botkit.createWebhookEndpoints = function (webserver, authenticationTokens) {
    ...
    webserver.post('/slack/receive', function (req, res) {
      ...
      if (req.body.command) {
        ...
        // recreate Slack data sent to server
  ...
  slack_botkit.setupWebserver = function (port, landingPageDir, cb) {
    ...
    slack_botkit.webserver.use(express.static(landingPageDir))
  ...
  slack_botkit.handleSlackEvents = function () {
    ...
    slack_botkit.on('message_received', function (bot, message) {
      ...
      if ('message' == message.type) {
        ...
        // console.log('Slackbot.js - handleSlackEvents - message.type')
